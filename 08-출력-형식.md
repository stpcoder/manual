...
본 시스템은 결과 저장 시 다음의 디렉토리 구조와 파일 형식으로 출력물을 생성한다. 모든 출력물은 사용자가 지정한 출력 폴더 하위에 생성된다.
...
원본 이미지 위에 검출된 손상 부위의 윤곽선을 오버레이한 비교 이미지이다.
...
검출된 각 손상 부위의 윤곽선을 개별 SVG 벡터 파일로 저장한 것이다.
...
레이저 커터에서 직접 사용할 수 있는 벡터 레이아웃 파일이다.
...
원본 이미지 위에 각 손상 부위의 번호를 오버레이한 가이드 이미지이다.

- 형식: PNG
- 크기: 원본 이미지와 동일
- 번호 표시: 각 조각의 중심 위치에 해당 번호를 표시
- 용도: 커팅된 보충 용지 조각을 원본 문서의 정확한 위치에 부착하기 위한 참조

커팅 레이아웃의 조각 번호와 복원 가이드의 위치 번호는 1:1로 대응되므로, 레이아웃에서 1번으로 커팅된 조각은 가이드에서 1번 위치에 부착한다.

### 8.5.2 piece_locations.csv

각 조각의 좌표 데이터를 CSV(Comma-Separated Values) 형식으로 기록한 파일이다.
...
전체 처리 결과의 메타데이터를 JSON 형식으로 기록한 파일이다. 입력 이미지 정보, 검출 설정, 레이아웃 설정, 배치 데이터를 포함한다.
...
```json
{
    ...
    "detection_settings": {
        ...
        "boundary_detection": "(논리값) 문서 경계 검출 사용 여부"
    },
...
세션 파일은 현재 작업 상태를 완전히 저장하여 이후 복원할 수 있도록 하는 JSON 파일이다. info.json이 최종 결과물의 메타데이터인 반면, 세션 파일은 작업 중간 상태를 포함한다.
...
시스템은 v1 및 v2 형식의 세션 파일도 로드할 수 있으며, 저장 시에는 항상 최신 버전(v3)으로 저장한다.
...
개별 손상 부위의 윤곽선은 SVG path의 d 속성 형식으로 표현된다. 사용되는 명령어는 다음과 같다.
...
상기 경로는 좌표 (100, 200)에서 시작하여 (150, 200), (150, 250), (100, 250)을 거쳐 시작점으로 돌아오는 직사각형을 나타낸다.
