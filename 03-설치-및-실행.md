...
본 시스템은 macOS 및 Linux에서도 동작 가능하나, 주요 테스트는 Windows 환경에서 수행되었다.
...
본 시스템이 요구하는 주요 의존성 패키지는 다음과 같다.
...
다음 패키지는 선택적(optional)이며, 딥러닝 기반 기능 사용 시에만 필요하다.
...
실행 시 PyQt6 기반의 그래픽 사용자 인터페이스가 나타나며, 다크 테마가 자동으로 적용된다.

### 3.3.2 실행 흐름

main.py의 실행 흐름은 다음과 같다.

1. QApplication 인스턴스를 생성한다.
2. 다크 테마 스타일시트를 적용한다(theme.py의 apply_theme 호출).
3. MainWindow 인스턴스를 생성하고 표시한다.
4. 이벤트 루프를 시작한다.
...
GUI 없이 명령줄에서 전체 파이프라인을 실행할 수 있다.
...
CLI 옵션의 상세 설명은 제6장 CLI 사용법을 참조한다.
...
build_exe.spec 파일의 주요 설정은 다음과 같다.
...
빌드된 실행 파일은 Python 환경이 설치되지 않은 시스템에서도 독립적으로 실행 가능하다. dist/HoleDetection/ 디렉토리 전체를 배포 대상 시스템에 복사한 후 HoleDetection.exe를 실행한다.

## 3.6 설치 확인

설치가 정상적으로 완료되었는지 확인하기 위해 다음을 수행한다.

1. GUI 실행 후 이미지를 드래그 앤 드롭하여 로드가 정상적으로 이루어지는지 확인한다.
2. "Apply Filter" 버튼을 클릭하여 검출이 수행되는지 확인한다.
3. "Layout" 탭으로 전환하여 레이아웃이 표시되는지 확인한다.

상기 과정에서 오류가 발생하는 경우, 의존성 패키지의 버전 호환성을 점검한다.

## 3.7 알려진 설치 관련 주의사항

### 3.7.1 한글 경로

프로젝트 디렉토리 및 이미지 파일의 경로에 한글이 포함된 경우 일부 기능에서 오류가 발생할 수 있다. 가능한 한 영문 경로를 사용할 것을 권장한다.

### 3.7.2 PyQt6 호환성

일부 Linux 배포판에서는 PyQt6의 시스템 의존성(libGL, libEGL 등)이 추가로 필요할 수 있다. 해당 환경에서는 배포판의 패키지 관리자를 통해 필요 라이브러리를 설치한다.
